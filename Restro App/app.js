...
var parentDiv = document.getElementById("reqItems");
var searchInput = document.getElementById("search-input");
var categoryFilter = "";

function renderMenu() {
  parentDiv.innerHTML = ""; // Clear existing menu items
  ...
  menu.forEach((item) => {
    if (
      ...
      childDiv.className = "menu-item col-12 col-md-4 col-lg-3 m-4";
      childDiv.innerHTML = `
                        <div class="card shadow">
                            <img src="${item.img}" class="card-img-top" alt="${item.title}" style="height: 240px;">
                            <div class="card-body">
                                <h5 class="card-title">${item.title}</h5>
                                <h6 class="card-subtitle mb-2 text-muted">${item.price}</h6>
                                <p class="card-text">${item.description}</p>
...
function setCategory(link) {
  categoryFilter = link.getAttribute("data-filter");
...
renderMenu();
// var childDiv = document.createElement("div");
// childDiv.innerHTML = menuItem(menu[0]);
